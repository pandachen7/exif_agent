...
class PhotoProcessor:
    ...
    def process_directory(self, directory: str) -> List[Dict]:
        ...
        for i, file_path in enumerate(files):
            self.logger.info(f"Processing file {i+1}/{len(files)}: {os.path.basename(file_path)}")

            record = self._process_single_file(
                file_path,
                csv_datetime_map,
                file_records
            )
    ...
    def _process_single_file(
        ...
        if not datetime_original:
            self.logger.warning(f"Could not determine datetime for {filename}, using 2000/1/1")
        ...
        record = {
            ...
            'Date': datetime_original.date(),
            'Time': datetime_original.time(),
        ...
        # 4. 檢查是否有多個動物標籤
        self._check_multiple_animal_tags(exif_data, filename)
        ...
        # 6. 如果沒有 Species 或 Species 為 unknown，則忽略
    ...
    def _determine_datetime(
        ...
        if filename in csv_datetime_map:
            try:
                ...
                self.logger.warning(f"Failed to parse CSV datetime for {filename}: {str(e)}")
    ...
    def _calculate_period_ranges(self, records: List[Dict], directory: str):
        ...
        for camera_id, group_records in camera_groups.items():
            ...
            self.logger.info(f"Camera {camera_id} period: {period_start} ~ {period_end}")
